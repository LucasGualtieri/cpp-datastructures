...
// #include "../include/graph/graph_builder.hpp"

// clear && g++ tests/adjacencyListTest.cc -std=c++20 -o bin/adjacencyListTest && bin/adjacencyListTest
...
void testAdjacencyList() {
	...
	assert(g.edges().size() / 2 == 2); // Two edges should exist
	...
	assert(gClone.edges().size() / 2 == 3); // Additional edge should exist in cloned graph
	assert(g.edges().size() / 2 == 2); // Original graph should remain unchanged
...
void speedTest() {
	...
	// {
	// 	Graph G = GraphBuilder()
	// 		.dataStructure(Graph::AdjacencyListPointers)
	// 	.build();
	//
	// 	const size_t NUM_VERTICES = 10000; // Number of vertices
	//
